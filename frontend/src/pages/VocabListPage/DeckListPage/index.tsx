    ...
    if (loading && hasMore) {
      ...
      setTimeout(() => {
        searchDecks(searchParams)
          .then((res) => {
            ...
            if (newDecks.length === 0) {
              ...
              if (newDecks[newDecks.length - 1].id === latestId) {
                ...
                console.log(
                  "new last id :" +
                    newDecks[newDecks.length - 1].id +
                    " , old id: " +
                    latestId
                );
                //변경 부분 중복 거르고 적용함
                const filteredNewDecks = newDecks.filter(
                  (deck) =>
                    !publicCardTitles.some(
                      (prevDeck) => prevDeck.id === deck.id
                    )
                );
                setLatestId(filteredNewDecks[filteredNewDecks.length - 1].id);
                setPublicCardTitles((prevDecks) => [
                  ...prevDecks,
                  ...filteredNewDecks,
                ]);
              }
          ...
          .catch((error) => {
            if (error.response.data.errorCode === "WORDS_003") {
              console.log("길이 0");
              setHasMore(false);
              return;
            }
