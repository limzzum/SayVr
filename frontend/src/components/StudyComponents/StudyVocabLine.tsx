...
export const StudyVocabLine: FC<VocabLineProps> = ({
  props,
  studyId,
  deckId,
}) => {
  ...
  useEffect(() => {
    setUncheckIcon(
      `unchecked-button ${
        status === WordcardStatus.UNCHECKED ? "unchecked" : ""
      }`
    );
    setCheckIcon(
      `checked-button ${status === WordcardStatus.CHECKED ? "checked" : ""}`
    );
              ...
              >
                <IconButton
                  ...
                  className={`checked-button ${
                    status === WordcardStatus.CHECKED ? "checked" : ""
                  }`}
                  onHover={false}
                />
              </div>
