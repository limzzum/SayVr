...
const WeeklySprintComponent: React.FC<WeeklySprintComponentProps> = ({
  ...
  const [unregisteredOptionTypes, setUnregisteredOptionTypes] = useState<
    OptionType[]
  >([]);
  const [selectedOptionType, setSelectedOptionType] = useState<
    OptionType | string
  >(
    unregisteredOptionTypes.length > 0
      ? unregisteredOptionTypes[0]
      : OptionType.ETC
  );
  const [editingGoal, setEditingGoal] = useState<GoalResponseDto | undefined>(
    undefined
  );
  ...
  useEffect(() => {
    const registeredOptionTypes = goalInfo?.goalDtoList?.map(
      (goal) => goal.optionType
    );
    ...
    if (registeredOptionTypes) {
      ...
      const newUnregisteredOptionTypes = allOptionTypes.filter(
        (optionType) => !registeredOptionTypes.includes(optionType)
      );
  ...
  const handleNextClick = () => {
    ...
    if (
      pageIndex <
      Math.ceil(
        goalInfo?.memberCheckListResponseDtoList.length / itemsPerPage
      ) -
        1
    ) {
      setPageIndex(pageIndex + 1);
  ...
  const onSubmit = (e: any) => {
    ...
    if (editing.status && editingGoal) {
      ...
      const updateGoalRequest: UpdateGoalResquestDto = {
        ...
        description: ["VR", "GAME", "QUIZ"].includes(editingGoal.optionType)
          ? undefined
          : value,
        count: ["VR", "GAME", "QUIZ"].includes(editingGoal.optionType)
          ? parseInt(value)
          : undefined,
      };
      updateGoal(
        studyId,
        goalInfo?.weeklySprintId,
        editing.id,
        updateGoalRequest
      )
        .then((res) => {
      ...
      const createGoalRequest: CreateGoalRequestDto = {
        ...
        description: ["VR", "GAME", "QUIZ"].includes(selectedOptionType)
          ? undefined
          : value,
        count: ["VR", "GAME", "QUIZ"].includes(selectedOptionType)
          ? parseInt(value)
          : undefined,
      };
    ...
    >
      ...
      ${new Date(
        new Date(goalInfo.targetDate).setDate(
          ...
          <div
            className="to-scroll"
            style={{ maxHeight: "80%", height: "70%", overflowY: "scroll" }}
          >
          ...
          {!weeklySprintStatus && studyRole === StudyRole.LEADER && (
            <button
              className={`circle-button-goal ${open ? "open" : ""}`}
      ...
      <div
        className="checklist-container-div"
        style={{ width: "69%", flex: 2 }}
      >
        <div
          className="checklist-container-inner"
